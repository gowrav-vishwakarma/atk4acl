...
class AclActionDecorator extends \atk4\ui\TableColumn\Generic {
	...
	public function init() {
		...
		$this->method_callback->set(function () use ($thisname) {
			...
			if ($this->acl_controller->model->hasMethod($action)) {
				...
				try {
					$this->acl_controller->db->connection->beginTransaction();
					$action_return = $this->acl_controller->model->{$action}();
					$this->acl_controller->db->connection->commit();
				} catch (\Exception $e) {
					$this->acl_controller->db->connection->rollback();
					throw $e;
				}
			} else {
			...
			if ($action_return === 'remove_row') {
				$atkjs = $reload->js()->find("[data-id=$model_id]")->transition('fade left')->jsRender();
			} else {
				$atkjs = (new \atk4\ui\jsReload($reload))->jsRender();
			}
			...
			$return = json_encode(['success' => true,
				...
				'html' => '',
				'id' => $reload->name]);
			$this->table->app->terminate($return);
		});
		...
		$this->page_callback->set(function ($page) use ($thisname) {
			...
			if (isset($_GET[$page->name . '_close']) && $page_return = $_GET[$page->name . '_close']) {
				$reload = $this->table->reload ?: $this->table;
				$atkjs = [(new \atk4\ui\jsExpression('$(".ui.active.modal").modal("hide all")'))->jsRender()];
				if ($page_return === 'remove_row') {
					$atkjs[] = $reload->js()->find("[data-id=$model_id]")->hide()->jsRender();
				} else {
					$atkjs[] = (new \atk4\ui\jsReload($reload))->jsRender();
				}
				$return = json_encode(['success' => true,
	...
	public function getHtmlTags($row, $field) {
		...
		foreach ($status_actions as $act => $detail) {
			if ($detail[0] == 'None' || ($detail['display'] !== 'page' && $detail['display'] !== 'method')) {
				continue;
			}

			if (($detail[0] == 'SelfOnly' || $this->acl_controller->isAssignField($status, $act)) && $row[$this->acl_controller->getConditionalField($status, $act)] != $this->acl_controller->app->auth->model->id) {
				continue;
			}
